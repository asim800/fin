"""
Risk metrics including VaR, CVaR, and tail risk measures.
"""
...
def calculate_var(returns: Union[pd.Series, np.ndarray],
    ...
    Returns:
    --------
    float: VaR (positive number representing potential loss)
    """
...
def calculate_cvar(returns: Union[pd.Series, np.ndarray],
    ...
    Returns:
    --------
    float: CVaR (positive number representing expected loss in tail)
    """
    returns = np.asarray(returns)
...
def calculate_ulcer_index(prices: Union[pd.Series, np.ndarray]) -> float:
    """
    Calculate Ulcer Index (depth and duration of drawdowns).
...
def calculate_tail_ratio(returns: Union[pd.Series, np.ndarray],
    ...
    returns : pd.Series or np.ndarray
        Return series
    percentile : float
        Percentile for tail measurement
...
def optimize_cvar_portfolio(expected_returns: np.ndarray,
    ...
    scenarios : np.ndarray
        Return scenarios (n_scenarios, n_assets)
    confidence : float
